...
// Reached only via the emailed link (?token=…) — never linked to from
// inside the app itself. No CrewAuthContext involvement: this runs fully
// logged-out, and on success just points the person at the ordinary sign-in
// screen rather than trying to establish a session itself.
export function CrewResetPassword() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSubmitting(true)
    try {
      await api.post('/crew/auth/reset-password', { token, new_password: newPassword })
      setDone(true)
    } catch (err) {
      ...
      <div style={{ width: '100%', maxWidth: 340, background: '#fff', border: '1px solid #DCE3E7', borderRadius: 16, padding: 28 }}>
        ...
        {!token ? (
          <div style={{ fontSize: 13.5, color: '#B42318', lineHeight: 1.5 }}>
            This reset link is missing its token. Request a new one from the <Link to="/crew/forgot-password" style={{ color: '#453E96' }}>forgot password</Link> page.
          </div>
        ) : done ? (
          <>
